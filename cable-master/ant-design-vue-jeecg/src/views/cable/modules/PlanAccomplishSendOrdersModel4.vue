        ...
        <a-row>
          <a-col :span="8">
            <a-form-item label="完单数量">
              <a-input v-decorator="['accomplishNum',validatorRules.accomplishNum]"
                       placeholder="请输入完单数量"></a-input>
            </a-form-item>
          </a-col>
          <a-col :span="8">
            <a-form-item label="单位">
              <a-select v-decorator="['accomplishNumUnit',validatorRules.accomplishNumUnit]"
                        placeholder="请选择单位">
                <a-select-option v-for="(unit,index) in unitList" :value="unit.itemValue">
                  {{unit.itemText}}
                </a-select-option>
              </a-select>
            </a-form-item>
          ...
          <a-col :span="8">
            <a-form-item label="实际回收规格">
              <a-select v-decorator="['recyclingSpecifications',validatorRules.recyclingSpecifications]"
                ...
                <a-select-option v-for="(item,index) in recyclingSpecifications"
          ...
          <template v-if="this.show">
            <a-col :span="8">
              <a-form-item label="是否异常">
                <a-select v-decorator="['sceneSituation',validatorRules.sceneSituation]"
                          placeholder="请选择是否异常"
                          @change="isException">
            ...
            <a-col :span="8">
              <a-form-item label="是否完成">
                <a-select v-decorator="['whetherComplete',validatorRules.whetherComplete]"
                          placeholder="请选择是否完成">
                  <a-select-option v-for="(item,index) in completeStates" :value="item.itemValue">
                    {{item.itemText}}
                  </a-select-option>
                </a-select>
              </a-form-item>
          ...
          <a-col :span="8">
            <a-form-item label="目标仓库">
              <a-select v-decorator="['warehouseId',validatorRules.warehouseId]" placeholder="请选择目标仓库"
                ...
                <a-select-option v-for="(item,index) in warehouseLists" :value="item.id">{{item.name}}
                </a-select-option>
              </a-select>
            </a-form-item>
          </a-col>
          <template v-if="this.show">
            <a-col :span="8">
              <a-form-item label="库位">
                <a-select v-decorator="['storageLocationId',validatorRules.storageLocationId]"
                          placeholder="请选择库位">
        ...
        <a-row>
          <template v-if="this.show">
            <a-col :span="16">
              <a-form-item label="项目报废物资入库交接单号">
                <a-input v-decorator="['receiptNo',validatorRules.receiptNo]"
                         placeholder="项目报废物资入库交接单号"></a-input>
              </a-form-item>
            </a-col>
          </template>
          <a-col :span="12" v-show="!this.show">
            <a-form-item label="电力仓库交接单号">
              <a-input v-decorator="['receiptNo',validatorRules.receiptNo]"
                       placeholder="电力仓库交接单号"></a-input>
            </a-form-item>
          </a-col>

          <a-col :span="12">
            <a-form-item label="回单">
              <j-image-upload text="点击上传"
                              v-decorator="['receiptPhotos',validatorRules.receiptPhotos]"></j-image-upload>
            </a-form-item>
          </a-col>
        </a-row>
          ...
          <a-row>
            <a-col :span="12">
              <a-form-item label="异常照片1">
                <j-image-upload text="点击上传"
                                v-decorator="['scenePhotos1',validatorRules.scenePhotos1]"></j-image-upload>
              </a-form-item>
            </a-col>
            <a-col :span="12">
