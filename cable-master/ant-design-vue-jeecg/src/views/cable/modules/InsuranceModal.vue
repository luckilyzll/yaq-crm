    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item label="车牌号码" :labelCol="labelCol" :wrapperCol="wrapperCol" class="cla">
          <j-search-select-tag placeholder="输入车牌号码" v-decorator="[ 'vehicleId',validatorRules.vehicleId]"
                               dict="vehicle,license,id" :async="true">
          </j-search-select-tag>
        </a-form-item>
        ...
        <a-form-item label="商业险保单号" :labelCol="labelCol" :wrapperCol="wrapperCol" class="cla">
          <a-input v-decorator="['insurancePolicy',validatorRules.insurancePolicy]"
                   placeholder="请输入商业险保单号"></a-input>
        </a-form-item>
        <a-form-item label="商业险日期" :labelCol="labelCol" :wrapperCol="wrapperCol" style="width: 610px;">
          <j-date style="width: 20%" v-decorator="['insuranceDateBegin',validatorRules.insuranceDateBegin]"
                  :showTime="true" date-format="YYYY-MM-DD" placeholder="请选择开始时间">
          </j-date>
          <span style="width: 10px">—</span>
          <j-date style="width: 20%" v-decorator="['insuranceDateEnd',validatorRules.insuranceDateEnd]"
                  :showTime="true" date-format="YYYY-MM-DD" placeholder="请选择结束时间">
          </j-date>
        </a-form-item>
        <a-form-item label="交强险保单号" class="cla" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input v-decorator="['strongPolicy',validatorRules.strongPolicy]"
                   placeholder="请输入交强险保单号"></a-input>
        </a-form-item>
        <a-form-item label="交强险日期" :labelCol="labelCol" :wrapperCol="wrapperCol" style="width: 610px;">
          <j-date style="width: 20%" v-decorator="['strongDateBegin',validatorRules.strongDateBegin]"
                  :showTime="true" date-format="YYYY-MM-DD" placeholder="请选择开始时间">
          </j-date>
          <span style="width: 10px">—</span>
          <j-date style="width: 20%" v-decorator="['strongDateEnd',validatorRules.strongDateEnd]"
                  :showTime="true" date-format="YYYY-MM-DD" placeholder="请选择结束时间">
          </j-date>
        </a-form-item>
      </a-form>
