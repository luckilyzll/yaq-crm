<template>
  <a-card :bordered="false">
          ...
          <a-col :md="4" :sm="7">
            <a-form-item label="类型">
              <a-select v-model="queryParam.planType" placeholder="请选择所属类型">
                <a-select-option v-for="(item,index) in plan3TypeList" :value="item.itemText">
                  {{item.itemText}}
                </a-select-option>
              </a-select>
            </a-form-item>
              ...
              <a-form-item label="完成状态">
                <a-select v-model="queryParam.completeState" placeholder="请选择完成状态">
                  <template v-for="(types,index) in completeStates">
                    <a-select-option v-bind:value="types.itemValue">{{types.itemText}}
                    </a-select-option>
                  </template>
                </a-select>
              </a-form-item>
    ...
    <plan-send-orders-the-same-day-modal ref="planSendOrdersTheSameDayModal"
                                         @ok="modasFormOk"></plan-send-orders-the-same-day-modal>
